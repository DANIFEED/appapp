...
def load_model():
    try:
        model = joblib.load('house_price_model.pkl.gz')
...
with st.sidebar:
    ...
    model = load_model()
    st.markdown("---")
    st.markdown("<h3 style='color: #2d3748;'>📊 Информация о модели</h3>", unsafe_allow_html=True)
    st.markdown("""
    <div style='
        background-color: white;
        padding: 1.5rem;
